...
const TeamManagement: React.FC = () => {
  ...
  // Since GetTeam by ID has issues, let's use the teams list to find the selected team
  const findSelectedTeam = (teamId: string) => {
    if (!teamId) {
  ...
  useEffect(() => {
    if (teams.length > 0) {
      findSelectedTeam(selectedTeamId);
    }
  ...
  const handleRemoveMember = async (teamId: number, memberId: number) => {
    ...
    try {
      ...
      // Refresh the teams list to get updated member information
      fetchTeams();
    } catch (err: any) {
